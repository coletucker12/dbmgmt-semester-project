const BASE_API_URL="https://semester-project-api.herokuapp.com"
...
const actions = {
  async FETCH_DATA({ commit }) {
    const reqs = [
      await this.$axios.get(`${BASE_API_URL}/api/students`),
      await this.$axios.get(`${BASE_API_URL}/api/courses`),
      await this.$axios.get(`${BASE_API_URL}/api/enrollments`)
    ]
